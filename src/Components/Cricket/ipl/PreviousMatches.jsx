...
export default function NextIplMatches() {
  ...
  const iplMatches =
    previousMatches?.typeMatches?.find(
      (match) => match.matchType === "League" || match.matchType === "Leagues"
    )?.seriesMatches?.[0]?.seriesAdWrapper?.matches || [];
  ...
  function formatOvers(overs) {
    const fullOvers = Math.floor(overs); // 14
    const balls = Math.round((overs - fullOvers) * 10); // 6 for 14.6, 4 for 14.4

    if (balls >= 6) {
      // handle overflow if any due to rounding errors
